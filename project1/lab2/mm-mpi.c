...
//		fprintf(stderr," --- SLAVE %d: Received row %d of matrix A\n", myid, row_id);
	}
...
//		fprintf(stderr," --- SLAVE %d: Received row %d of matrix B\n", myid, i);
	}
	after = wall_clock_time();
	comm_time += after - before;
...
//			fprintf(stderr," +++ MASTER : Sending row %d of matrix A to process %d\n", row_id, slave_id);
		}
	}
...
//		fprintf(stderr," +++ MASTER : Sending row %d of matrix B to all slaves\n", i);
	}
}
